...
public class FrameSequenceDrawable extends Drawable implements Animatable, Runnable {
    ...
    /**
     * 正确姿势，但是稍微耗时和内存
     * 初始化FrameSequenceState
     * 初始化FrontBitmap
     * 调用FrameSequenceState.getFrame(0, FrontBitmap, -1);
     * 这个接口在部分情况下可能会耗内存（因为bitmap）
     * （1）图像已经开始进行渲染
    ...
    public Bitmap getFirstFrame() {
        ...
        if (mFrameSequenceState != null) {
            ...
            try {
                ...
                mFrameSequenceState.getFrame(0, bitmap, -1);
                return bitmap;
        ...
        /**
         * 下面的代码如果没有其他异常不要打开，未经过测试。
         * 如果FrameSequence.State 出现问题，我们可以自己初始化一个完成解码
         * **/
        /*
        if(mFrameSequence != null) {
            FrameSequence.State state = mFrameSequence.createState();
            final int width = mFrameSequence.getWidth();
            final int height = mFrameSequence.getHeight();
            try {
                Bitmap bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
